...
def _load_overrides(target, override_list, found_fn):
    overrides = devpipeline_core.config.override.read_overrides(
        devpipeline_core.config.paths.get_overrides_root(_CONFIG_DIR),
        target, override_list)
    return devpipeline_core.config.override.apply_all_overrides(
        overrides, found_fn)
